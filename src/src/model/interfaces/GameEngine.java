...
public interface GameEngine {
	 ...
	 * 
	 * 1. deal a card to the player 2. call GameEngineCallback.nextCard(...) 3.
	 * continue looping until the player busts (default value of
	 * GameEngine.BUST_TOTAL=21) 4. GameEngineCallback.bustCard(...) 5. call
	 * {@link GameEngineCallback#result(Player, int, GameEngine)} with final result
	 * for player (the pre bust total) 6. update the player with final result so it
	 * can be retrieved later
	 * 
	 * @param player the current player who will have their result set at the end of
	 *               the hand
	...
	/**
	 * Same as dealPlayer() but deals for the house and calls the house versions of
	 * the callback methods on GameEngineCallback, no player parameter is required
	 * 
	 * After the house deal has finished but BEFORE calling houseResult() win/loss
	 * values are applied to all players based on their bet
	 * 
	 * @see view.interfaces.GameEngineCallback
	 * 
	 * @param delay the delay between cards being dealt
	...
	/**
	 * @param gameEngineCallback a client specific implementation of
	 *                           GameEngineCallback used to perform display updates
	 *                           etc.
	 * 
	...
	/**
	 * @param gameEngineCallback called when a player quits the game to remove no
	 *                           longer needed UI updates
	 ...
	 * @return true if bet is greater than or equal to 0 and player had sufficient
